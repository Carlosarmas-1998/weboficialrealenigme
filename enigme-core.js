  ...
  function initScreenshotDetection() {
    ...
    (function blockScreenshots() {
      ...
      document.addEventListener('keydown', function(e) {
        ...
        // Ctrl+Shift+S, Ctrl+Shift+I, Ctrl+S, Cmd+Shift+3/4/5 (Mac)
        if ((e.ctrlKey || e.metaKey) && e.shiftKey && (e.key === 's' || e.key === 'S' || e.key === 'i' || e.key === 'I' || e.key === '3' || e.key === '4' || e.key === '5')) {
          e.preventDefault();
          showBlockMessage();
          return false;
        }
      ...
      var style = document.createElement('style');
      style.textContent = '\n' +
        'img, video, .product-card, .carousel-wrapper { \n' +
      ...
      function showBlockMessage() {
        var overlay = document.getElementById('enigme-block-overlay');
        if (overlay) { overlay.style.display = 'flex'; return; }
        overlay = document.createElement('div');
        overlay.id = 'enigme-block-overlay';
        overlay.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.92);z-index:999999;display:flex;flex-direction:column;align-items:center;justify-content:center;color:#c9a96e;font-family:Cormorant Garamond,serif;text-align:center;padding:20px;';
        overlay.innerHTML = '<div style="font-size:3rem;margin-bottom:20px;">\u2726</div><h2 style="font-size:1.8rem;margin-bottom:15px;letter-spacing:3px;">CONTENIDO PROTEGIDO</h2><p style="font-size:1rem;color:#999;max-width:400px;line-height:1.6;font-family:Jost,sans-serif;">Esta imagen es propiedad exclusiva de La Casa \u00C8NIGME.<br><br>\u00BFTe encanta? Adqui\u00E9relo y hazlo tuyo.</p><button onclick="this.parentElement.style.display=\'none\'" style="margin-top:30px;padding:12px 40px;background:#c9a96e;color:#000;border:none;font-family:Jost,sans-serif;font-size:0.85rem;letter-spacing:2px;cursor:pointer;text-transform:uppercase;">CERRAR</button>';
        document.body.appendChild(overlay);
      }
      window.showBlockMessage = showBlockMessage;
    })();
  }

  function showScreenshotMessage() {
    // Delegate to new blocking system
    if (window.showBlockMessage) {
      window.showBlockMessage();
    }
  }
